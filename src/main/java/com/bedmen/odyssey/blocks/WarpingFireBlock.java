...
public class WarpingFireBlock extends AbstractWarpingFireBlock {
    ...
    public WarpingFireBlock() {
        super(AbstractBlock.Properties.create(Material.FIRE, MaterialColor.TNT).doesNotBlockMovement().zeroHardnessAndResistance().setLightLevel((state) -> { return 15; }).sound(SoundType.CLOTH), 1.0F);
        ...
        }).collect(Collectors.toMap(Function.identity(), WarpingFireBlock::getShapeForState)));
    }
    ...
    private BlockState getFireWithAge(IWorld world, BlockPos pos, int age) {
        ...
        return blockstate.isIn(BlockRegistry.WARPING_FIRE.get()) ? blockstate.with(AGE, Integer.valueOf(age)) : blockstate;
    }
    ...
    public static void init() {
        WarpingFireBlock fireblock = (WarpingFireBlock) BlockRegistry.WARPING_FIRE.get();
