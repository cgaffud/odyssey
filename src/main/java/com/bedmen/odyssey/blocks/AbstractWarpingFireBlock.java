...
public abstract class AbstractWarpingFireBlock extends Block {
    private final float fireDamage;
    protected static final VoxelShape shapeDown = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 16.0D, 1.0D, 16.0D);

    public AbstractWarpingFireBlock(AbstractBlock.Properties properties, float fireDamage) {
        super(properties);
    ...
    public static BlockState getFireForPlacement(IBlockReader reader, BlockPos pos) {
        ...
        return SoulFireBlock.shouldLightSoulFire(blockstate.getBlock()) ? Blocks.SOUL_FIRE.getDefaultState() : ((WarpingFireBlock)BlockRegistry.WARPING_FIRE.get()).getStateForPlacement(reader, pos);
    }
