...
public class AlloyFurnaceBlock extends ContainerBlock {
    ...
    public AlloyFurnaceBlock() {
        super(AbstractBlock.Properties.of(Material.STONE).requiresCorrectToolForDrops().strength(3.5F).lightLevel((state) -> {return state.getValue(BlockStateProperties.LIT) ? 13 : 0;}));
    ...
    public void onRemove(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
        if (!state.is(newState.getBlock())) {
            TileEntity tileentity = worldIn.getBlockEntity(pos);
            if (tileentity instanceof AlloyFurnaceTileEntity) {
                InventoryHelper.dropContents(worldIn, pos, (AlloyFurnaceTileEntity)tileentity);
                ((AlloyFurnaceTileEntity)tileentity).grantStoredRecipeExperience(worldIn, Vector3d.atCenterOf(pos));
