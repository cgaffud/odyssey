...
public class OdysseyEnchantingTableBlock extends Block implements ITileEntityProvider {
    protected static final VoxelShape SHAPE = Block.box(0.0D, 0.0D, 0.0D, 16.0D, 12.0D, 16.0D);

    public OdysseyEnchantingTableBlock() {
    ...
    public TileEntity newBlockEntity(IBlockReader worldIn) {
        return new OdysseyEnchantingTableTileEntity();
    }
    ...
    public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (worldIn.isClientSide) {
            ...
            if (tileentity instanceof OdysseyEnchantingTableTileEntity) {
                player.openMenu((OdysseyEnchantingTableTileEntity)tileentity);
            }
    ...
    public void setPlacedBy(World worldIn, BlockPos pos, BlockState state, LivingEntity placer, ItemStack stack) {
        if (stack.hasCustomHoverName()) {
            TileEntity tileentity = worldIn.getBlockEntity(pos);
            if (tileentity instanceof OdysseyEnchantingTableTileEntity) {
                ((OdysseyEnchantingTableTileEntity)tileentity).setCustomName(stack.getHoverName());
            }
        }
    ...
    public void onRemove(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
        if (!state.is(newState.getBlock())) {
            TileEntity tileentity = worldIn.getBlockEntity(pos);
            if (tileentity instanceof OdysseyEnchantingTableTileEntity) {
                InventoryHelper.dropContents(worldIn, pos, (OdysseyEnchantingTableTileEntity)tileentity);
