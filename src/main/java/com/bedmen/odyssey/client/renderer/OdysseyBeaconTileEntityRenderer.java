...
@OnlyIn(Dist.CLIENT)
public class OdysseyBeaconTileEntityRenderer extends TileEntityRenderer<OdysseyBeaconTileEntity> {
    public static final ResourceLocation TEXTURE_BEACON_BEAM = new ResourceLocation("textures/entity/beacon_beam.png");

    public OdysseyBeaconTileEntityRenderer(TileEntityRendererDispatcher rendererDispatcherIn) {
        super(rendererDispatcherIn);
    }

    public void render(OdysseyBeaconTileEntity tileEntityIn, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn, int combinedOverlayIn) {
        long i = tileEntityIn.getLevel().getGameTime();
        List<OdysseyBeaconTileEntity.BeamSegment> list = tileEntityIn.getBeamSegments();
        int j = 0;

        for(int k = 0; k < list.size(); ++k) {
            OdysseyBeaconTileEntity.BeamSegment beacontileentity$beamsegment = list.get(k);
    ...
    public boolean shouldRenderOffScreen(OdysseyBeaconTileEntity te) {
        return true;
    }
}
