...
public interface IBossEventEntity {
    ...
    default int getNearbyPlayerNumber(){
        ...
        return serverBossInfo.getPlayers().stream().filter(serverPlayerEntity -> {return serverPlayerEntity.isAlive() && !serverPlayerEntity.isInvulnerable();}).collect(Collectors.toList()).size();
    }
