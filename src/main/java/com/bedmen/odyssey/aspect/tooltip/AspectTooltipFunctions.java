...
public class AspectTooltipFunctions {
    ...
    public static final AspectTooltipFunction NUMBER_ADDITION = (input -> Component.literal(StringUtil.additiveFloatFormat(input.aspectInstance().strength)+" ").append(input.aspectInstance().aspect.getComponent()));
    public static final AspectTooltipFunction PER_SECOND = (input -> Component.literal(StringUtil.additiveFloatFormat(input.aspectInstance().strength * 20f)+" ").append(input.aspectInstance().aspect.getComponent()).append(Component.translatable("aspect_tooltip.oddc.per_second")));
    public static final AspectTooltipFunction BONUS_DAMAGE = (input -> Component.literal(StringUtil.additiveFloatFormat( input.aspectInstance().strength * (input.optionalItemStack().isPresent() ? BonusDamageAspect.getStrengthAmplifier(input.optionalItemStack().get().getItem()) : 1.0f))+" ").append(input.aspectInstance().aspect.getComponent()));
    public static final AspectTooltipFunction PERCENTAGE_ADDITION = (input -> Component.literal(StringUtil.additivePercentFormat(input.aspectInstance().strength)+" ").append(input.aspectInstance().aspect.getComponent()));
    public static final AspectTooltipFunction HP_THRESHHOLD = (input -> Component.literal(StringUtil.additiveFloatFormat(input.aspectInstance().strength)+"HP ").append(input.aspectInstance().aspect.getComponent()));
    ...
    public static final AspectTooltipFunction ADDITIONAL_SHIELD_DAMAGE_BLOCK = (input -> Component.literal(StringUtil.additiveFloatFormat(input.aspectInstance().strength)+" ").append(input.aspectInstance().aspect.getComponent()));
}
