...
public class AspectTooltipFunctions {
    ...
    public static final AspectTooltipFunction ADDITIONAL_SHIELD_DAMAGE_BLOCK = (input -> Component.literal("+"+StringUtil.floatFormat(AspectShieldItem.getDifficultyAdjustedDamageBlock(input.aspectInstance().strength, input.optionalLevel().map(LevelAccessor::getDifficulty).orElse(Difficulty.NORMAL)))+" ").append(input.aspectInstance().aspect.getComponent()));
}
