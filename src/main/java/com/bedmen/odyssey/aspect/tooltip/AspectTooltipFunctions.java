...
public class AspectTooltipFunctions {
    ...
    // Boolean
    public static final AspectTooltipFunction<Boolean> NAME = (input -> input.aspectInstance().aspect.getComponent());
    public static final AspectTooltipFunction<Boolean> NAME_AND_ACTIVATION_KEY = (input -> input.aspectInstance().aspect.getComponent().append(Component.translatable("aspect.oddc.activation_key", ((ActivationAspect)input.aspectInstance().aspect).getKeyboardKey())));
    ...
    public static final AspectTooltipFunction<Float> MULTISHOT = (input -> Component.literal( MultishotAspect.strengthToNumberOfSideProjectiles(input.aspectInstance().value)+" ").append(input.aspectInstance().aspect.getComponent()).append(Component.translatable("aspect.oddc.multishot.damage_penalty", StringUtil.percentFormat(MultishotAspect.strengthToDamagePenalty(input.aspectInstance().value)))));
