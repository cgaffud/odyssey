...
public class AspectTooltipFunctions {
    ...
    public static final AspectTooltipFunction MULTISHOT = (input -> new TextComponent( MultishotAspect.strengthToNumberOfSideProjectiles(input.aspectInstance().strength)+" ").append(input.aspectInstance().aspect.getComponent()).append(new TranslatableComponent("aspect.oddc.multishot.damage_penalty", StringUtil.percentFormat(MultishotAspect.strengthToDamagePenalty(input.aspectInstance().strength)))));
