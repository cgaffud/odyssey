...
public class AspectBowItem extends BowItem implements INeedsToRegisterItemModelProperty, AspectItem, OdysseyRangedAmmoWeapon {
    ...
    public AspectBowItem(Item.Properties properties, float damageMultiplier, int baseMaxChargeTicks, List<AspectInstance> abilityList, List<AspectInstance> innateModifierList) {
        super(properties);
    ...
    public void onUseTick(Level level, LivingEntity livingEntity, ItemStack itemStack, int count) {
        if(this.getUseDuration(itemStack) - count > WeaponUtil.getRangedMaxChargeTicks(itemStack) && AspectUtil.hasBooleanAspect(itemStack, Aspects.REPEAT)){
            ...
            if (!ammo.isEmpty() || flag) {
                ...
                if (useTicks >= 3) {
                    ...
                    if (!level.isClientSide) {
                        ...
                        float inaccuracy = 1.0f / AspectUtil.getUnitAspectValue(bow, Aspects.ACCURACY);
