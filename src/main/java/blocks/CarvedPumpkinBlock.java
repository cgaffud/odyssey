...
public class CarvedPumpkinBlock extends HorizontalBlock implements IArmorVanishable {
   ...
   @Nullable
   private BlockPattern rubyGolemBasePattern;
   @Nullable
   private BlockPattern rubyGolemPattern;
   ...
   public CarvedPumpkinBlock() {
      super(AbstractBlock.Properties.create(Material.GOURD, MaterialColor.ADOBE).hardnessAndResistance(1.0F).sound(SoundType.WOOD));
   ...
   public boolean canDispenserPlace(IWorldReader reader, BlockPos pos) {
      return this.getSnowmanBasePattern().match(reader, pos) != null || this.getGolemBasePattern().match(reader, pos) != null || this.getRubyGolemBasePattern().match(reader, pos) != null;
   }
      ...
      if (blockpattern$patternhelper != null) {
         ...
         if (blockpattern$patternhelper != null) {
            ...
            for(int i1 = 0; i1 < this.getGolemPattern().getPalmLength(); ++i1) {
        	 ...
        	 blockpattern$patternhelper = this.getRubyGolemPattern().match(world, pos);
             if (blockpattern$patternhelper != null) {
                 for(int j = 0; j < this.getRubyGolemPattern().getPalmLength(); ++j) {
                    for(int k = 0; k < this.getRubyGolemPattern().getThumbLength(); ++k) {
                       CachedBlockInfo cachedblockinfo2 = blockpattern$patternhelper.translateOffset(j, k, 0);
                       world.setBlockState(cachedblockinfo2.getPos(), Blocks.AIR.getDefaultState(), 2);
                       world.playEvent(2001, cachedblockinfo2.getPos(), Block.getStateId(cachedblockinfo2.getBlockState()));
                    }
                 ...
                 BlockPos blockpos = blockpattern$patternhelper.translateOffset(1, 2, 0).getPos();
                 RubyGolemEntity rubygolementity = (RegistryHandler.RUBY_GOLEM.get()).create(world);
                 rubygolementity.setPlayerCreated(true);
                 rubygolementity.setLocationAndAngles((double)blockpos.getX() + 0.5D, (double)blockpos.getY() + 0.05D, (double)blockpos.getZ() + 0.5D, 0.0F, 0.0F);
                 world.addEntity(rubygolementity);

                 for(ServerPlayerEntity serverplayerentity1 : world.getEntitiesWithinAABB(ServerPlayerEntity.class, rubygolementity.getBoundingBox().grow(5.0D))) {
                    CriteriaTriggers.SUMMONED_ENTITY.trigger(serverplayerentity1, rubygolementity);
                 }

                 for(int i1 = 0; i1 < this.getRubyGolemPattern().getPalmLength(); ++i1) {
                    for(int j1 = 0; j1 < this.getRubyGolemPattern().getThumbLength(); ++j1) {
                       CachedBlockInfo cachedblockinfo1 = blockpattern$patternhelper.translateOffset(i1, j1, 0);
                       world.func_230547_a_(cachedblockinfo1.getPos(), Blocks.AIR);
                    }
                 }
   ...
   private BlockPattern getRubyGolemBasePattern() {
	      if (this.rubyGolemBasePattern == null) {
	         this.rubyGolemBasePattern = BlockPatternBuilder.start().aisle("~ ~", "###", "~#~").where('#', CachedBlockInfo.hasState(BlockStateMatcher.forBlock(RegistryHandler.RUBY_BLOCK.get()))).where('~', CachedBlockInfo.hasState(BlockMaterialMatcher.forMaterial(Material.AIR))).build();
	      }

	      return this.rubyGolemBasePattern;
	   }

   private BlockPattern getRubyGolemPattern() {
      if (this.rubyGolemPattern == null) {
         this.rubyGolemPattern = BlockPatternBuilder.start().aisle("~^~", "###", "~#~").where('^', CachedBlockInfo.hasState(IS_PUMPKIN)).where('#', CachedBlockInfo.hasState(BlockStateMatcher.forBlock(RegistryHandler.RUBY_BLOCK.get()))).where('~', CachedBlockInfo.hasState(BlockMaterialMatcher.forMaterial(Material.AIR))).build();
      }

      return this.rubyGolemPattern;
   }
}
