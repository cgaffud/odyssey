...
public class AlloyFurnaceBlock extends ContainerBlock {
	...
	public AlloyFurnaceBlock() {
		super(AbstractBlock.Properties.create(Material.ROCK).setRequiresTool().hardnessAndResistance(3.5F).setLightLevel((state) -> {return state.get(BlockStateProperties.LIT) ? 13 : 0;}));
	...
	public void onBlockPlacedBy(World worldIn, BlockPos pos, BlockState state, LivingEntity placer, ItemStack stack) {
	      if (stack.hasDisplayName()) {
	         TileEntity tileentity = worldIn.getTileEntity(pos);
	         if (tileentity instanceof AlloyFurnaceTileEntity) {
	            ((AlloyFurnaceTileEntity)tileentity).setCustomName(stack.getDisplayName());
	         }
	      }
	   }
	...
	public void onReplaced(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
	      if (!state.isIn(newState.getBlock())) {
	         TileEntity tileentity = worldIn.getTileEntity(pos);
	         if (tileentity instanceof AlloyFurnaceTileEntity) {
	            InventoryHelper.dropInventoryItems(worldIn, pos, (AlloyFurnaceTileEntity)tileentity);
	            ((AlloyFurnaceTileEntity)tileentity).grantStoredRecipeExperience(worldIn, Vector3d.copyCentered(pos));
